<template>
  <div class="main-content">
    <breadcumb :page="'Gestión de Fisioterapeutas'"/>
          ...
          <b-row>
            ...
            <!-- <b-col md="6" sm="12">
              <validation-provider name="role" :rules="{ required: true}">
                <b-form-group slot-scope="{ valid, errors }" :label="'Rol'">
                  <v-select
    ...
    <validation-observer ref="Create_User">
      <b-modal hide-footer size="lg" id="Details" :title="'Horarios'">
        <b-row>
          <div v-for="(day, index) in dayss" :key="index" class="mt-1">
              <b-card>
                ...
                <b-collapse :visible="visibleDays.includes(day)">
                  <div>
                    <div v-for="(input, index) in slots[day]">
                      <input  class='form-control' :key="index" type="time" v-model="input.time" placeholder="Ingrese las horas">
...
export default {
  ...
  methods: {
    ...
    async Details(user) {
      ...
      this.user.NewPassword = null;
      this.editmode = true;
      let data = await axios.post('Appointments/turns', {});
      data = data.data.turns;
      for(let day of this.dayss){
        for(let turn of data){
          if(day == turn.day)
            this.slots[day].push({time: turn.time}); 
        }
      }
